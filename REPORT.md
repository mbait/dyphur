...
- **Coloring is itself parallel** (Jones–Plassmann/Luby). Each round, a constraint joins an independent set iff it holds the highest *priority* among the uncoloured constraints at both its dynamic bodies, then takes the lowest colour free of its already-coloured neighbours (per-body 64-bit used-colour mask; spill → serial fallback if > 64 colours). Priority = `(hash(index) << 32 | index)` — pseudo-random to break index chains (so coloring finishes in ~O(log n) rounds), with the index in the low bits guaranteeing a unique maximum (⇒ the set is independent, the colour assignment is correct, and the whole thing is deterministic). This replaced an earlier **single-work-item greedy** coloring that scanned all constraints on one GPU thread; profiling showed *that serial pass*, not the parallel solve sweeps, was the solver's dominant cost at scale (5.2 ms of an 11 ms solve at 8 192 bodies — it scales linearly with constraint count on one thread). Parallelising it cut the solver ~3× at 8k (§9.4). A naïve min-index round rule was tried first and was catastrophic (long spatial index chains ⇒ > 64 rounds ⇒ fallback): the random priority is load-bearing.
- **Quaternion renormalisation** moved to its own per-body parallel kernel (was per-iteration inside the serial kernel).
- **Motor pre-pass** (PD joint drive) is skipped entirely when the scene has no joints — otherwise it dispatched `n_colors × n_con` no-op work-items per frame.
...
Result: the solver dropped from ~11 ms to ~3.7 ms at 8 192 bodies; `stress_test_blocks` runs **~135 fps warm at 8 192 bodies** (was ~78) and the GPU beats the OpenMP CPU. Result hashes change whenever the coloring changes (colour order ≠ contact-index order); the colored path is gated on run-to-run determinism, not a fixed golden.
...
- **The colored solver was the #1 cost — but its bottleneck was the *coloring*, not the solve.** The original table read 64–77 % in the solve stage; a sub-profile (below) found ~5 ms of an 11 ms solve at 8k was the **single-work-item greedy coloring** (one GPU thread, scattered memory, linear in constraint count), with the actual GS sweeps only ~0.3 ms/iteration. Parallelising the coloring (Jones–Plassmann, §4.5.1) cut the solver to ~3.7 ms at 8k (≈3×) and is what took the 8k frame to ~135 fps warm.
...
**Attacking `build_and_query` (the #2 cost).** A per-step sub-profile (each of the seven build steps fenced) at 8 192 bodies put the cost almost entirely in the **top-down pair-query traversal (~70 %)**, with the Morton sort second (~18 %) and the refit third (~9 %); Morton/Karras/root steps are negligible. The traversal is **memory-latency-bound**, not control-flow- or atomic-bound — confirmed by two negative experiments: a *stackless rope (escape-pointer) traversal* (removing the per-thread 64-entry stack) gave **no speed-up**, and switching the per-pair counter from a `seq_cst` to a relaxed atomic also gave none. The cost is the per-node AABB fetch: traversal visits nodes in tree order (random in memory) and each visit read six fp16 bounds from **six separate arrays** — up to six scattered cache-line loads per node. Packing the bounds into one 16-byte `NodeBox` (§4.3) makes each visit a single 128-bit load, cutting `build_and_query` ~9–15 % and speeding ray-query traversal for free, with **bit-identical results** (same fp16 values, only the layout changed — the 1 025-body hash is unchanged). The remaining cost is the inherent per-leaf root descent, which re-tests the upper tree once per leaf — measured at **>4–5× more node visits than emitted pairs** at 8k.

A **BVTT front** (bounding-volume test tree) was then implemented to remove that redundancy: a front of node-pairs propagated one level per kernel, seeded at `(root, root)`, descending overlapping internal-node pairs and emitting overlapping leaf pairs — each leaf-pair enumerated once, so the candidate set (and the hash) is identical. It *is* faster where the redundancy is largest (8k `build_and_query` 2.37 → 2.10 ms, −11 %), but it is a **net regression below ~6k bodies** (1k 0.84 → 1.34 ms, +60 %; 4k +6 %): propagating the front needs ~30 **per-level host syncs** (each kernel must read the next front's count to size the following launch), a fixed ~0.5 ms cost the per-leaf query — contention-free, sync-free, perfectly parallel — never pays. Since the win is confined to the largest scenes and `build_and_query` is no longer the dominant stage, the BVTT front was **not kept**; the per-leaf query remains. (A megakernel/persistent-threads front would avoid the per-level syncs but needs device-wide synchronisation AdaptiveCpp does not portably expose.)

**Attacking the colored solver (the #1 cost).** Halving the iteration count (10 → 5) cut the 8k solve only 11.1 → 9.2 ms — so the per-iteration GS work is just ~0.3 ms/iter and there is a **~7–8 ms fixed cost independent of iterations**. Timing the stages directly pinned it on the **single-work-item greedy coloring**: 5.2 ms at 8k (one GPU thread scanning ~51 k constraints with scattered `inv_mass`/mask reads), scaling linearly with constraint count. The coloring is now parallel (Jones–Plassmann/Luby, §4.5.1): the solver dropped to ~3.7 ms at 8k (≈3×), 8k warm throughput went **~78 → ~135 fps**, and the solver is no longer the clear bottleneck. Two negative results bracketed the win: dispatching each colour over a **compact per-colour bucket** (instead of rescanning all constraints with an early-out) was a *regression* — the GPU skips early-returning threads cheaply, and building the buckets cost more than it saved; and a **min-index** parallel-coloring round rule was catastrophic — spatially-ordered indices form long chains needing > 64 rounds. The random (hashed) priority in Jones–Plassmann is the load-bearing detail. Results change with the coloring (different colour order ⇒ different Gauss–Seidel schedule), so the colored path is gated on run-to-run determinism (which holds ×3 at 1k/4k/8k), not a fixed golden; small jointed scenes use the unchanged serial path, so the manipulator grasp hash is untouched.

### 9.5 Optimisation history — the migrating bottleneck

GPU performance was reached by a sequence of targeted passes, each of which moved the dominant cost to the next stage rather than uniformly speeding everything up. Recording the progression matters because it shows *where* the bottleneck lived at each step and why the next change was the right one — the same single-frame profile that once read "77 % sort," then "68 % solver," now spreads ~46 % solve / ~30 % `build_and_query` / ~12 % sort at 8 192 bodies, with no single stage dominating. After the coloring fix the 8k frame runs ~135 fps warm (from ~55 at the start of this work).
...
| `build_and_query` traversal | top-down pair query **~70 %** of `build_and_query`, **memory-latency-bound** (rope traversal and relaxed atomics both gave nothing) | Pack the six fp16 node bounds into one 16-byte `NodeBox` ⇒ one 128-bit load per node visit | `build_and_query` −9–15 %; ray-query faster for free; bit-identical results. Also fixed a latent bug: the Morton sort's GPU fast path was gated by a compile-time macro, so the CUDA build silently ran the barrier-broken fast path on `Device::default_cpu()` (the determinism gate) — now gated at runtime by device type. A follow-up **BVTT front** (removing the >4–5× per-leaf descent redundancy) was implemented and measured but **not kept**: it helps only at ≥ ~6k bodies (8k −11 %) and regresses smaller scenes (1k +60 %) because of ~30 per-level host syncs |
| Colored solver | **single-work-item greedy coloring** = ~5 ms of the 11 ms solve at 8k (a `n_iters` sweep is only ~0.3 ms) | Parallel Jones–Plassmann/Luby coloring with hashed priorities; skip motor pre-pass when no joints | solver ~11 → ~3.7 ms at 8k (≈3×); **8k ~78 → ~135 fps** warm. Bucketing the per-colour dispatch and a min-index round rule were both tried and rejected (regression / non-convergent) |
